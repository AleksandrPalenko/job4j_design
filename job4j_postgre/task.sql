...
--1(1)
select p.name, p.company_id from person p left join company c on p.company_id = c.id
where c.id != 5

--1(2)
select c.name, p.name from company c left join person p on c.id = p.company_id;
